...
// Evaluate a polynomial with order x
    ...
    if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          Eigen::Map<Eigen::VectorXd> eig_waypoints_x(ptrx, 6);
          Eigen::Map<Eigen::VectorXd> eig_waypoints_y(ptry, 6);

          auto coeffs = polyfit(eig_waypoints_x, eig_waypoints_y, 3);
          double cte = polyeval(coeffs, 0); // px = 0, py = 0
          double epsi = -atan(coeffs[1]);   // p

          // Calculate steer and throttle values with MPC
          ...
          std::cout << "steering_angle: " << steer_value << ", throttle: " << throttle_value << endl;
